...
namespace Microsoft.AspNetCore.Components.Rendering
{
    ...
    public class HtmlRenderer : Renderer
    {
        ...
        /// <param name="htmlEncoder">The <see cref="HtmlEncoder"/> to use for encoding content.</param>
        public HtmlRenderer(IServiceProvider serviceProvider, HtmlEncoder htmlEncoder) : base(serviceProvider)
        {
            ...
            int length)
        {
            ...
            switch (frame.FrameType)
            {
                ...
                case RenderTreeFrameType.Text:
                    result.Add(_htmlEncoder.Encode(frame.TextContent));
                    return ++position;
            ...
            ArrayRange<RenderTreeFrame> frames, int position, int maxElements)
        {
            ...
            for (var i = 0; i < maxElements; i++)
            {
                ...
                if (frame.AttributeValue is bool flag && flag)
                {
                    result.Add(" ");
                    result.Add(frame.AttributeName);
                }

                if (frame.AttributeValue is string value)
                {
                    result.Add(" ");
                    result.Add(frame.AttributeName);
                    result.Add("=");
